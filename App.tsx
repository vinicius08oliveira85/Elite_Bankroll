...
const App: React.FC = () => {
  ...
  // FUNÇÃO DE EXPORTAÇÃO (BACKUP DO SISTEMA)
  const handleExportBackup = () => {
    ...
    link.href = url;
    link.download = `elite_bankroll_backup_${new Date().getTime()}.json`;
    link.click();
  ...
  const stats = useMemo(() => {
    ...
    timeline.forEach(item => {
      ...
      // Atualizar Pico Histórico
      if (currentBalance > peakBalance) {
        peakBalance = currentBalance;
      }
      
      // Calcular Drawdown Atual em relação ao Pico
      if (peakBalance > 0) {
        const drawdown = ((peakBalance - currentBalance) / peakBalance) * 100;
        if (drawdown > maxDDPercent) {
          maxDDPercent = drawdown;
        }
      }
    });
    ...
    const categoryMap: Record<string, { profit: number, wins: number, total: number, stake: number }> = {};
    allOps.forEach(op => {
      const cat = (op.category || 'GERAL').toUpperCase();
      if (!categoryMap[cat]) categoryMap[cat] = { profit: 0, wins: 0, total: 0, stake: 0 };
    ...
    const categoryStats = Object.entries(categoryMap).map(([name, d]) => ({
      name,
      profit: d.profit,
  ...
  const requestAiAdvisor = async () => {
    ...
    try {
      ...
      const prompt = `Você é um Mentor Financeiro Especialista em Gestão de Risco para Traders Esportivos.
      Contexto Auditado:
      - Saldo Atual: R$${stats.currentBalance.toFixed(2)}
      - Drawdown Máximo: ${stats.maxDrawdown.toFixed(2)}%
      - ROI Global: ${stats.roi.toFixed(1)}%
      - Pior Categoria: ${stats.worstCategory?.name || 'N/A'} (Lucro: R$${stats.worstCategory?.profit.toFixed(2) || 0})
      
      Forneça um conselho pragmático sobre gestão de banca e como mitigar o drawdown atual. Seja direto. Máximo 2 frases.`;
      ...
      setAiAnalysis("Erro na auditoria de IA. Verifique sua conexão.");
    } finally {
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.3em]">Operational Risk & Psychology v4.2</p>
            </div>
          </div>
          <div className="flex gap-3 w-full md:w-auto">
            <button onClick={handleExportBackup} title="Backup do Sistema (JSON)" className="flex items-center justify-center gap-2 p-3 bg-slate-900 rounded-2xl border border-slate-800 hover:bg-slate-800 transition shadow-lg text-slate-400 hover:text-emerald-400 px-4">
              <Download size={18}/> <span className="text-[10px] font-black uppercase hidden sm:block">Backup</span>
            </button>
            ...
            <button onClick={() => setIsTxModalOpen(true)} className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-slate-900 px-6 py-3 rounded-2xl font-black border border-slate-800 transition hover:bg-slate-800 shadow-lg font-mono text-xs uppercase"><Wallet size={18}/> Caixa</button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-slate-900/60 border border-slate-800 rounded-[2rem] p-6 shadow-xl backdrop-blur-md">
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1">Patrimônio Atual</p>
          ...
          <div className={`bg-slate-900/60 border rounded-[2rem] p-6 shadow-xl backdrop-blur-md transition-all duration-500 ${
            stats.maxDrawdown > 25 ? 'border-rose-500/50 shadow-rose-900/10' : stats.maxDrawdown > 15 ? 'border-amber-500/50 shadow-amber-900/10' : 'border-slate-800'
          }`}>
            <div className="flex justify-between items-center mb-1">
              <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Drawdown Máximo</p>
              <AlertTriangle size={14} className={stats.maxDrawdown > 15 ? (stats.maxDrawdown > 25 ? 'text-rose-500 animate-pulse' : 'text-amber-500 animate-pulse') : 'text-slate-600'} />
            </div>
            <h2 className={`text-3xl font-black italic ${stats.maxDrawdown > 25 ? 'text-rose-500' : stats.maxDrawdown > 15 ? 'text-amber-500' : 'text-cyan-400'}`}>
              -{stats.maxDrawdown.toFixed(2)}%
            </h2>
            <p className="text-slate-500 text-[10px] mt-2 font-bold uppercase tracking-tighter">Risco Operacional Histórico</p>
          </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-[2rem] p-6 shadow-xl backdrop-blur-md">
            ...
            <p className="text-slate-500 text-[10px] mt-2 font-bold uppercase">Consistência Estratégica</p>
          </div>

          <div className="bg-slate-900/60 border border-slate-800 rounded-[2rem] p-6 shadow-xl backdrop-blur-md">
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1">ROI Global</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-slate-900/40 border border-slate-800 rounded-[2.5rem] p-8 h-[450px] shadow-2xl backdrop-blur-xl">
               <div className="flex justify-between items-center mb-8">
                  <h3 className="font-black uppercase text-xs tracking-[0.3em] text-slate-500 flex items-center gap-3">
                    <TrendingUp size={16} className="text-emerald-500"/> Equity Growth Curve
                  </h3>
               </div>
            ...
            <div className="bg-slate-900/40 border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl backdrop-blur-xl">
              <div className="p-8 border-b border-slate-800/60 flex justify-between items-center bg-slate-900/20">
                <h3 className="font-black uppercase text-xs tracking-[0.3em] text-slate-500 italic flex items-center gap-2"><History size={16}/> Histórico de Ordens</h3>
                ...
                <table className="w-full">
                  <thead className="sticky top-0 bg-slate-900/90 backdrop-blur-md text-[10px] text-slate-500 uppercase font-black tracking-widest z-10">
                    <tr>
                      ...
                      <th className="px-8 py-5 text-center">Sentimento</th>
                      ...
                      <tr key={op.id} className="hover:bg-slate-800/20 transition group">
                        ...
                        <td className="px-8 py-5 text-center">
                           {op.isPositiveEV ? 
                            ...
                            <span className="text-slate-600 bg-slate-800 px-2 py-1 rounded text-[9px] font-black tracking-widest italic border border-slate-700/50">NO VALUE</span>}
                        </td>
                        <td className="px-8 py-5 text-center">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 shadow-2xl">
               <h3 className="font-black text-indigo-400 uppercase text-[10px] tracking-[0.4em] mb-8 flex items-center gap-3 italic">
                 <BarChart3 size={18}/> Market Performance
               </h3>
               
               <div className="space-y-6">
                  <div>
                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                      <Trophy size={12} className="text-emerald-500"/> Alpha Zone (Melhor)
                    </p>
                    {stats.bestCategory ? (
                      ...
                      <p className="text-[10px] text-slate-700 italic px-4">Aguardando dados...</p>
                    )}
                  </div>

                  <div>
                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                      <Skull size={12} className="text-rose-500"/> Risk Zone (Pior)
                    </p>
                    {stats.worstCategory && stats.worstCategory.profit < 0 ? (
                      ...
                      <p className="text-[10px] text-slate-700 italic px-4">Performance estável.</p>
                    )}
                  </div>
               </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-indigo-900/10 to-slate-950 border border-indigo-500/20 rounded-[2.5rem] p-8 space-y-6 relative overflow-hidden group">
               ...
               <p className="text-sm text-slate-300 leading-relaxed italic font-medium">
                 {aiAnalysis || "Solicite uma auditoria para correlacionar seu Drawdown com seu pior mercado."}
               </p>
               <button 
               ...
               >
                 {isAnalyzing ? "AUDITANDO VETORES..." : "CONSULTAR ESTRATEGISTA"}
               </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-950/98 backdrop-blur-2xl flex items-center justify-center p-4 z-50">
          <form onSubmit={handleAddOperation} className="bg-slate-900 border border-slate-800 rounded-[3rem] p-10 max-w-lg w-full space-y-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Execução de Registro</p>
            </div>
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                 <input name="probability" type="number" required value={previewProb} onChange={(e) => setPreviewProb(e.target.value)}
                    className="w-full bg-slate-950/50 border border-slate-800 rounded-2xl px-5 py-4 font-black text-indigo-400 outline-none text-xl font-mono" placeholder="Confiança %" />
              ...
              <div className="p-4 bg-slate-950/80 rounded-2xl border border-slate-800 flex justify-between items-center">
                 <div>
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Cálculo +EV:</span>
                 ...
                 <div className="text-right">
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">R$ Alocados:</span>
              ...
              <select name="status" className="w-full bg-slate-950/50 border border-slate-800 rounded-2xl px-5 py-4 font-black outline-none appearance-none text-center tracking-widest uppercase text-xs">
                 <option value="GREEN" className="text-emerald-500">FECHAR COM WIN (GREEN)</option>
                 <option value="RED" className="text-rose-500">FECHAR COM LOSS (RED)</option>
                 <option value="PENDING">MANTER ABERTA (LIVE)</option>
              </select>
            </div>
      ...
      {isConfigOpen && (
        <div className="fixed inset-0 bg-slate-950/98 backdrop-blur-2xl flex items-center justify-center p-4 z-50">
          <form onSubmit={updateConfig} className="bg-slate-900 border border-slate-800 rounded-[3rem] p-10 max-w-md w-full space-y-8 relative shadow-2xl">
            ...
            <h2 className="text-3xl font-black uppercase text-cyan-400 italic">Arquitetura</h2>
            <div className="space-y-6">
          ...
          <form onSubmit={(e) => {
            ...
            <div className="space-y-6">
              <select name="type" className="w-full bg-slate-950/50 border border-slate-800 rounded-2xl px-6 py-4 font-black outline-none appearance-none tracking-widest text-xs uppercase">
                <option value="DEPOSIT">DEPÓSITO (IN)</option>
            ...
            <button type="submit" className="w-full bg-white text-slate-950 hover:bg-slate-200 py-6 rounded-2xl font-black uppercase text-sm transition-all shadow-xl active:scale-95">Registrar Ordem</button>
          </form>
        </div>
      )}
    </div>
