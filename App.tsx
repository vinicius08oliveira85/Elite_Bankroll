...
const App: React.FC = () => {
  ...
  const handleExportBackup = () => {
    ...
    a.href = url;
    a.download = `elite_backup_${new Date().getTime()}.json`;
    a.click();
    URL.revokeObjectURL(url);
  };

  // ESTATÍSTICAS E GESTÃO DE RISCO AVANÇADA
  const stats = useMemo(() => {
    ...
    timeline.forEach(item => {
      ...
      if (peakBalance > 0) {
        const currentDD = ((peakBalance - runningBalance) / peakBalance) * 100;
        if (currentDD > maxDDPercent) {
          maxDDPercent = currentDD;
        }
      }
    });
    ...
    const unitValue = (runningBalance * bankroll.unitValuePercent) / 100;
    ...
    const categoryMap: Record<string, { profit: number, stake: number, count: number, wins: number }> = {};
    allOps.forEach(op => {
      const cat = (op.category || 'GERAL').toUpperCase();
      if (!categoryMap[cat]) categoryMap[cat] = { profit: 0, stake: 0, count: 0, wins: 0 };
    ...
    const worstCategory = [...categoryStats].sort((a, b) => a.roi - b.roi)[0] || null;
  ...
  const requestTacticalAdvisor = async () => {
    setIsAnalyzing(true);
    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      const prompt = `Você é um Mentor de Risco para investidores de alto desempenho.
        Diagnóstico Institucional:
        - Patrimônio: R$${stats.currentBalance.toFixed(2)}
        - Max Drawdown: ${stats.maxDrawdown.toFixed(2)}%
        - ROI Atual: ${stats.roi.toFixed(1)}%
        - Pior Mercado: ${stats.worstCategory?.name || 'N/A'} (ROI: ${stats.worstCategory?.roi.toFixed(1) || 0}%)
        
        Sua missão é dar um conselho direto e agressivo para proteger o capital. Se o Drawdown estiver acima de 15%, seu tom deve ser de alerta máximo. Foque em como mitigar perdas no pior mercado. Máximo 2 frases curtas.`;
      ...
      setAiAnalysis("Erro na auditoria de IA. Verifique sua chave de acesso.");
    } finally {
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.3em]">Operational Risk Controller v4.2 PRO</p>
            </div>
          </div>
          <div className="flex gap-3 w-full md:w-auto">
            <button onClick={handleExportBackup} title="Exportar Backup JSON" className="flex items-center justify-center gap-2 p-3 bg-slate-900 rounded-2xl border border-slate-800 hover:bg-slate-800 transition shadow-lg text-slate-400 hover:text-emerald-400 px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className={`bg-slate-900/60 border rounded-[2rem] p-6 shadow-xl backdrop-blur-md transition-all duration-500 ${
            stats.maxDrawdown > 25 ? 'border-rose-500/50 shadow-rose-950/10' : stats.maxDrawdown > 15 ? 'border-amber-500/50 shadow-amber-950/10' : 'border-slate-800'
          }`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-slate-900/40 border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl backdrop-blur-xl">
              <div className="p-8 border-b border-slate-800/60 flex justify-between items-center bg-slate-900/20">
                <h3 className="font-black uppercase text-xs tracking-[0.3em] text-slate-500 italic flex items-center gap-2"><History size={16}/> Histórico Auditado</h3>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 shadow-2xl">
               <h3 className="font-black text-cyan-400 uppercase text-[10px] tracking-[0.4em] mb-8 flex items-center gap-3 italic">
               ...
               <div className="space-y-6">
                  <div>
                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                      <Trophy size={12} className="text-emerald-500"/> Alpha Zone (Melhor ROI)
                    </p>
                    {stats.bestCategory ? (
                  ...
                  <div>
                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                      <Skull size={12} className="text-rose-500"/> Risk Zone (Pior ROI)
                    </p>
                    {stats.worstCategory && stats.worstCategory.profit < 0 ? (
                      ...
                      <p className="text-[10px] text-slate-700 italic px-4">Performance estável em todos mercados.</p>
                    )}
                  </div>
               </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-indigo-900/10 to-slate-950 border border-indigo-500/20 rounded-[2.5rem] p-8 space-y-6 relative overflow-hidden group">
               ...
               <p className="text-sm text-slate-300 leading-relaxed italic font-medium">
                 {aiAnalysis || "Solicite uma auditoria institucional para correlacionar seu Drawdown com seu pior mercado e estancar fugas de capital."}
               </p>
               <button 
                onClick={requestTacticalAdvisor} 
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                 <input name="probability" type="number" required value={previewProb} onChange={(e) => setPreviewProb(e.target.value)}
                    className="w-full bg-slate-950/50 border border-slate-800 rounded-2xl px-5 py-4 font-black text-indigo-400 outline-none text-xl font-mono" placeholder="Prob %" />
              ...
              <div className="p-4 bg-slate-950/80 rounded-2xl border border-slate-800 flex justify-between items-center">
                 ...
                 <div className="text-right">
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">R$ Alocado:</span>
      ...
      {isConfigOpen && (
        <div className="fixed inset-0 bg-slate-950/98 backdrop-blur-2xl flex items-center justify-center p-4 z-50">
          <form onSubmit={updateConfig} className="bg-slate-900 border border-slate-800 rounded-[3rem] p-10 max-w-md w-full space-y-8 relative shadow-2xl">
            ...
            <button type="submit" className="w-full bg-cyan-600 hover:bg-cyan-500 py-6 rounded-2xl font-black uppercase text-sm shadow-xl active:scale-95 transition-all">Aplicar Estratégia</button>
          </form>
        </div>
      )}
